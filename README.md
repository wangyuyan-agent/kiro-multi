...
| macOS asks for the `security` / `login` keychain password | a per-profile keychain is locked, usually from an older build or a usage-refresh path that touched Kiro auth before unlocking it | upgrade to ≥ v0.2.6; both `kiro-wrap` sessions and usage refreshes unlock the profile keychain with the expected empty password and reapply no-timeout settings |
